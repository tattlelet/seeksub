...
pub const AtDepthArrayTokenizer2 = struct {
    input: []const u8,
    cursor: usize = 0,
    valueStart: usize = 0,
    state: State = .noop,
    ...
    pub fn next(self: *@This()) Error!?[]const u8 {
        return self.next2() catch |e| {
            std.debug.print("Error {s}, Input ({s}), stack {d}, index {d}, at {s}\n", .{
                @errorName(e),
                self.input,
                self.stack,
                self.cursor,
                if (self.cursor < self.input.len) self.input[self.cursor..] else "EOF",
            });
            return e;
        };
    }
    ...
    const State = struct {
        ...
        fn consume(self: *State, i: usize, c: u8) State.Error!void {
            ...
            switch (c) {
                ...
                // NOTE: this acctualy accepts a lot more than it should
                else => {
                    switch (tag) {
                        .noop => return State.Error.MissingArrayLayer,
...
test "simple array iterator" {
    const t = std.testing;
    var iterator = AtDepthArrayTokenizer2.init("[1,2,3]");
...
fn tstCollectTokens2(allocator: *const Allocator, slice: []const u8) ![]const []const u8 {
    var result = try std.ArrayListUnmanaged([]const u8).initCapacity(allocator.*, slice.len);
    var tokenizer = AtDepthArrayTokenizer2.init(slice);
...
test "Depth 1 Array tokenizer (non-string)" {
    const t = std.testing;
    const E = AtDepthArrayTokenizer.Error || AtDepthArrayTokenizer2.Error;
    const base = &std.testing.allocator;
    var arena = std.heap.ArenaAllocator.init(base.*);
    defer arena.deinit();
    const allocator = &arena.allocator();

    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "]"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, " ]"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "] "));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, " ] "));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "\t]"));
    ...
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "\t]\t"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, " \t] \t "));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "1"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, " 1 "));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "\t1\t"));

    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "1,2"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "1, 2"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, " 1 , 2 "));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "\t1,\t2\t"));
    ...
    try t.expectEqualDeep(expectEmpty, try tstCollectTokens2(allocator, ""));
    try t.expectEqualDeep(expectEmpty, try tstCollectTokens2(allocator, " "));
    try t.expectEqualDeep(expectEmpty, try tstCollectTokens2(allocator, "  "));
    try t.expectEqualDeep(expectEmpty, try tstCollectTokens2(allocator, "\t"));
    try t.expectEqualDeep(expectEmpty, try tstCollectTokens2(allocator, "\t "));
    ...
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "["));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, " ["));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[ "));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "\t["));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[\t"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "\t[ \t"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, " \t[ \t "));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1,"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, " [1,"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1, "));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "\t[1,"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1,\t"));

    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, ]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,  ]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,\t]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,\t\t]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, \t]"));
    ...
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[ ,]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[  ,]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[\t,]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[,\t]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[ \t,\t ]"));
    ...
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, ,\t]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,\t,\t]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,,1]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, ,1]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,\t,1]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, ,\t1]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,\t,\t1]"));
    ...
    try t.expectEqualDeep(expectOne, try tstCollectTokens2(allocator, "[1]"));
    try t.expectEqualDeep(expectOne, try tstCollectTokens2(allocator, "[ 1]"));
    try t.expectEqualDeep(expectOne, try tstCollectTokens2(allocator, "[1 ]"));
    try t.expectEqualDeep(expectOne, try tstCollectTokens2(allocator, "[ 1 ]"));
    try t.expectEqualDeep(expectOne, try tstCollectTokens2(allocator, "[\t1]"));
    ...
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[1,1]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[1, 1]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[1 ,1]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[1 , 1]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[ 1,1]"));
...
test "Depth 2+ Array tokenizer (non-string)" {
    ...
    try t.expectEqualDeep(expectOneEmpty, try tstCollectTokens2(allocator, "[[]]"));
    try t.expectEqualDeep(expectOneEmpty, try tstCollectTokens2(allocator, " [[]]"));
    try t.expectEqualDeep(expectOneEmpty, try tstCollectTokens2(allocator, "[[]] "));
    try t.expectEqualDeep(expectOneEmpty, try tstCollectTokens2(allocator, " [[]] "));
    try t.expectEqualDeep(expectOneEmpty, try tstCollectTokens2(allocator, "\t[[]]\t"));
    ...
    try t.expectEqualDeep(expectOneSpace, try tstCollectTokens2(allocator, "[ [ ] ]"));
    try t.expectEqualDeep(expectOneSpace, try tstCollectTokens2(allocator, " [ [ ] ]"));
    try t.expectEqualDeep(expectOneSpace, try tstCollectTokens2(allocator, "[ [ ] ] "));
    try t.expectEqualDeep(expectOneSpace, try tstCollectTokens2(allocator, " [ [ ] ] "));
    try t.expectEqualDeep(expectOneSpace, try tstCollectTokens2(allocator, "\t[ [ ] ]\t"));
    ...
    try t.expectEqualDeep(expectOneRaw1, try tstCollectTokens2(allocator, "[[1,]]"));
    try t.expectEqualDeep(expectOneRaw1, try tstCollectTokens2(allocator, " [[1,]] "));
    try t.expectEqualDeep(expectOneRaw1, try tstCollectTokens2(allocator, "\t[[1,]]\t"));
    try t.expectEqualDeep(expectOneRaw1Space, try tstCollectTokens2(allocator, "[[1, ]]"));
    try t.expectEqualDeep(expectOneRaw1Space, try tstCollectTokens2(allocator, " [[1, ]] "));
    try t.expectEqualDeep(expectOneRaw1Space, try tstCollectTokens2(allocator, "\t[[1, ]]\t"));
    try t.expectEqualDeep(expectMultipleRaw, try tstCollectTokens2(allocator, "[[],[1],[2,3]]"));
    try t.expectEqualDeep(expectMultipleRaw, try tstCollectTokens2(allocator, " [[],[1],[2,3]] "));
    try t.expectEqualDeep(expectMultipleRaw, try tstCollectTokens2(allocator, "\t[[],[1],[2,3]]\t"));
    try t.expectEqualDeep(expectMultipleRaw, try tstCollectTokens2(allocator, "[ [], [1], [2,3] ]"));
    try t.expectEqualDeep(expectMultipleRaw, try tstCollectTokens2(allocator, " [ [], [1], [2,3] ] "));
    ...
    try t.expectEqualDeep(expectOneRawNested, try tstCollectTokens2(allocator, "[[[]]]"));
    try t.expectEqualDeep(expectOneRawNested, try tstCollectTokens2(allocator, " [[[]]] "));
    try t.expectEqualDeep(expectOneRawNested, try tstCollectTokens2(allocator, "\t[[[]]]\t"));
    ...
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[[[]]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[[1,]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[[], [1], [2,3]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, " [ [ ]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[[1], [2,]"));

    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[1, 2], ]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[1, 2], [3],]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[], , [1]]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[1, 2], , [3]]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[], , [1]]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[[],  , [1]]"));
    ...
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[[]] ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[[]]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[[]] ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[[]]] "));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, " [ []]]"));
    ...
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ [ ] ] ] "));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "\t[[]]]\t"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1] ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1]] "));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, " [1]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ 1 ] ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ 1 ] ] "));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "\t[1]]\t"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1, 2]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1, 2] ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1, 2]] "));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, " [1, 2]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ 1, 2 ] ]"));
    ...
    try t.expectEqualDeep(expectMixedDepth, try tstCollectTokens2(allocator, "[1, [3, 4], 2, [[4], 4]]"));

    const expectMixedDepthSpaces: []const []const u8 = &.{ "1", "[ 3 , 4 ]", "2", "[[4], 4]" };
    try t.expectEqualDeep(expectMixedDepthSpaces, try tstCollectTokens2(allocator, "[ 1 , [ 3 , 4 ] , 2 , [[4], 4] ]"));

    const expectMixedDepthTabs: []const []const u8 = &.{ "1", "[3,\t4]", "2", "[[4],\t4]" };
    try t.expectEqualDeep(expectMixedDepthTabs, try tstCollectTokens2(allocator, "[1,\t[3,\t4],2,[[4],\t4]]"));

    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1, [3, 4], 2, [[4], 4]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1, [3, 4], 2, [[4], 4"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "[1, [3, 4], 2, [[4], 4],"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1, [3, 4], , 2, [[4], 4]]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[1,, [3, 4], 2, [[4], 4]]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[1, [3, 4], 2, [[4], 4]]]"));
    try t.expectError(E.MissingArrayLayer, tstCollectTokens2(allocator, "1, [3, 4], 2, [[4], 4]"));
}
...
test "Depth 1 Array tokenizer (strings)" {
    ...
    try t.expectEqualDeep(expectEmptyString, try tstCollectTokens2(allocator, "[ '' ]"));
    const expectSpaceString: []const []const u8 = &.{" "};
    try t.expectEqualDeep(expectSpaceString, try tstCollectTokens2(allocator, "[ ' ' ]"));
    const expectWhitespaceString: []const []const u8 = &.{"  "};
    try t.expectEqualDeep(expectWhitespaceString, try tstCollectTokens2(allocator, "[ '  ' ]"));
    const expectTabString: []const []const u8 = &.{"\t"};
    try t.expectEqualDeep(expectTabString, try tstCollectTokens2(allocator, "[ '\t' ]"));
    const expectTabSpaceString: []const []const u8 = &.{"\t "};
    try t.expectEqualDeep(expectTabSpaceString, try tstCollectTokens2(allocator, "[ '\t ' ]"));
    const expectMixedWhitespaceString: []const []const u8 = &.{" \t\t "};
    try t.expectEqualDeep(expectMixedWhitespaceString, try tstCollectTokens2(allocator, "[ ' \t\t ' ]"));

    const expectTwo: []const []const u8 = &.{ "a", "b" };
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "['a','b']"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[ 'a', 'b' ]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[ 'a' , 'b' ]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[ 'a'\t,\t'b' ]"));
    try t.expectEqualDeep(expectTwo, try tstCollectTokens2(allocator, "[\t'a'\t,\t'b'\t]"));
    ...
    try t.expectEqualDeep(expectArrayStrings, try tstCollectTokens2(allocator, "['[1,2]', '[3,4]']"));
    try t.expectEqualDeep(expectArrayStrings, try tstCollectTokens2(allocator, "[ '[1,2]' , '[3,4]' ]"));
    try t.expectEqualDeep(expectArrayStrings, try tstCollectTokens2(allocator, "[\t'[1,2]',\t'[3,4]'\t]"));
    try t.expectEqualDeep(expectArrayStrings, try tstCollectTokens2(allocator, "[ '[1,2]', '[3,4]' ]"));

    const expectArrayInsideString: []const []const u8 = &.{"[1,2]"};
    try t.expectEqualDeep(expectArrayInsideString, try tstCollectTokens2(allocator, "['[1,2]']"));

    const expectNestedArrayInString: []const []const u8 = &.{"[[1,2]]"};
    try t.expectEqualDeep(expectNestedArrayInString, try tstCollectTokens2(allocator, "['[[1,2]]']"));

    const expectArrayAndCommaInString: []const []const u8 = &.{"[1, 2], [3,4]"};
    try t.expectEqualDeep(expectArrayAndCommaInString, try tstCollectTokens2(allocator, "['[1, 2], [3,4]']"));

    const expectMixed: []const []const u8 = &.{ "a", "[1,2]", "b" };
    try t.expectEqualDeep(expectMixed, try tstCollectTokens2(allocator, "['a', '[1,2]', 'b']"));

    const expectMoreMixed: []const []const u8 = &.{ "[a]", "[b,c]", "x", "y" };
    try t.expectEqualDeep(expectMoreMixed, try tstCollectTokens2(allocator, "['[a]', '[b,c]', 'x', 'y']"));

    const expectSpaced: []const []const u8 = &.{ "[ 1 , 2 ]", "[ 3 , 4 ]" };
    try t.expectEqualDeep(expectSpaced, try tstCollectTokens2(allocator, "[ '[ 1 , 2 ]' , '[ 3 , 4 ]' ]"));
    try t.expectEqualDeep(expectSpaced, try tstCollectTokens2(allocator, "[\t'[ 1 , 2 ]',\t'[ 3 , 4 ]'\t]"));

    const expectUnbalancedBracketsInString: []const []const u8 = &.{"[[[["};
    try t.expectEqualDeep(expectUnbalancedBracketsInString, try tstCollectTokens2(allocator, "['[[[[']"));

    const expectMixedBracketInString2: []const []const u8 = &.{ "[w,]", "[", "]" };
    try t.expectEqualDeep(expectMixedBracketInString2, try tstCollectTokens2(allocator, "['[w,]', '[', ']']"));

    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "['a]"));
    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "[ 'a ]"));
    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "['a', 'b]"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "['a', 'b'"));

    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[''w]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ 'a'b ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "[ 'abc'1 ]"));
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "['it'broken']"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[, 'a']"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "['a', , 'b']"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "['a', ]"));
    try t.expectError(E.EmptyCommaSplit, tstCollectTokens2(allocator, "[ 'a',    ]"));

    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "['a',"));
    try t.expectError(E.UnexpectedEndOfInput, tstCollectTokens2(allocator, "['a'"));
}
...
test "Depth 2+ Array tokenizer (strings)" {
    ...
    try t.expectEqualDeep(expectInnerQuoted, try tstCollectTokens2(allocator, "[[1, 'a'], [2, 'b']]"));

    const expectQuotedOnlyInner: []const []const u8 = &.{ "['a','b']", "['c','d']" };
    try t.expectEqualDeep(expectQuotedOnlyInner, try tstCollectTokens2(allocator, "[['a','b'], ['c','d']]"));

    const expectNestedMix: []const []const u8 = &.{ "[1, 'a', [2, 'b']]", "x" };
    try t.expectEqualDeep(expectNestedMix, try tstCollectTokens2(allocator, "[[1, 'a', [2, 'b']], 'x']"));

    const expectDeepStringInside: []const []const u8 = &.{ "[[ 'abc', [1,2] ]]", "tail" };
    try t.expectEqualDeep(expectDeepStringInside, try tstCollectTokens2(allocator, "[[[ 'abc', [1,2] ]], 'tail']"));

    const expectSpacedQuoted: []const []const u8 = &.{ "[ 1 , 'a' ]", "[ 'b' , 2 ]" };
    try t.expectEqualDeep(expectSpacedQuoted, try tstCollectTokens2(allocator, "[ [ 1 , 'a' ] , [ 'b' , 2 ] ]"));

    const expectMixedWhitespace: []const []const u8 = &.{ "[1,\t'a']", "[\t'b',2]" };
    try t.expectEqualDeep(expectMixedWhitespace, try tstCollectTokens2(allocator, "[ [1,\t'a'] , [\t'b',2] ]"));
}
...
test "Any depth any match tricky cases" {
    ...
    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "['a', 'b']['c', 'd']"));
    // Sanitization of inner-level doesn't happen at top-level
    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "[[1, 2], [3, 4']"));

    const expectTricky1: []const []const u8 = &.{ "['a,b', 'c']", "['d', 'e']" };
    try t.expectEqualDeep(expectTricky1, try tstCollectTokens2(allocator, "[['a,b', 'c'], ['d', 'e']]"));

    const expectTricky2: []const []const u8 = &.{"['[1,2]', '[3,4]']"};
    try t.expectEqualDeep(expectTricky2, try tstCollectTokens2(allocator, "[['[1,2]', '[3,4]']]"));

    const expectTricky3: []const []const u8 = &.{"['1, 2], [2,3']"};
    try t.expectEqualDeep(expectTricky3, try tstCollectTokens2(allocator, "[['1, 2], [2,3']]"));

    const expectTricky4: []const []const u8 = &.{"1, 2], [2,3"};
    try t.expectEqualDeep(expectTricky4, try tstCollectTokens2(allocator, "['1, 2], [2,3']"));

    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "['[1, 2]', [3, '4]"));

    const expectTricky5: []const []const u8 = &.{ "1", "2, 3], [4, 5" };
    try t.expectEqualDeep(expectTricky5, try tstCollectTokens2(allocator, "[1, '2, 3], [4, 5']"));

    try t.expectError(E.SyntaxError, tstCollectTokens2(allocator, "['[a, b'],"));

    const expectTricky6: []const []const u8 = &.{ "['[x, y], z']", "['a, b]']" };
    try t.expectEqualDeep(expectTricky6, try tstCollectTokens2(allocator, "[['[x, y], z'], ['a, b]']]"));

    try t.expectError(E.EarlyQuoteTermination, tstCollectTokens2(allocator, "[['1, 2'], ['3]]"));

    const expectTricky7: []const []const u8 = &.{ "1", "2", "3, [4, 5]" };
    try t.expectEqualDeep(expectTricky7, try tstCollectTokens2(allocator, "[1, 2, '3, [4, 5]']"));
}
